...
if ($old_pass != null && $new_pass != null && $re_pass != null) {
    ...
    if (password_verify($old_pass, $db->getHash($user))) {
        ...
        if ($new_pass == $re_pass) {
                ...
                $status = "<p class='success'>Password successfully changed!</p>";
        } else {
            $status = "<p class='warning'>Password doesn't match!</p>";
        }
    } else {
        $status = "<p class='warning'>Old password is not correct!</p>";
    }
} else {
